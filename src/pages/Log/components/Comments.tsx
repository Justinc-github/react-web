...
export default function Comments() {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/comments?content_id=${contentId}`
      );
  ...
  const renderComment = (comment: Comment) => {
    const maxAllowedDepth = 2; // 最大缩进层级

    // 计算实际显示的缩进层级（超过3级时保持为3级）
    const displayDepth = Math.min(comment.level, maxAllowedDepth);
    const indent = displayDepth * 32;

    // 边框只显示在前3级
    const showBorder = comment.level > 1 && comment.level <= maxAllowedDepth;

    const imageUrls = normalizeImages(comment.images);

    return (
      <div
        key={`${comment.id}-${comment.level}`}
        className={`py-3 ${comment.level > 1 ? "border-top" : ""}`}
        style={{
          marginLeft: `${indent}px`,
          borderLeft: showBorder ? "2px solid #e9ecef" : "none",
